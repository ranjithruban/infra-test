...
resource "docker_container" "newsfeed-1" {
    image = "${docker_image.newsfeed-1.latest}"
    name = "newsfeed"
    hostname = "newsfeed"
    env = ["APP_PORT=8002"]
...
resource "docker_image" "newsfeed-1" {
  name = "ranjithruban/newsfeed:latest"
}

# Start quotes container
resource "docker_container" "quotes-1" {
    image = "${docker_image.quotes-1.latest}"
    name = "quotes"
    hostname = "quotes"
    env = ["APP_PORT=8001"]
...
resource "docker_image" "quotes-1" {
  name = "ranjithruban/quotes:latest"
}

# Start static-asset container
resource "docker_container" "static-assets-1" {
    image = "${docker_image.static-assets-1.latest}"
    name = "static-assets"
...
resource "docker_container" "frontend-1" {
    ...
    links = [
    "${docker_container.newsfeed-1.name}",
    "${docker_container.quotes-1.name}",
    "${docker_container.static-assets-1.name}"
  ]
    must_run="true"
}

# Find the newfeed image.
resource "docker_image" "frontend-1" {
